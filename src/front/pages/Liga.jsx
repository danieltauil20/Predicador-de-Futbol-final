...
export const Liga = () => {
  ...
  const getLogoPath = (nombre) => nombre ? `/logos/${MAPPING_EQUIPOS[nombre.trim()] || 'default'}.png` : "/logos/default.png";
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ display: "flex", gap: "10px", justifyContent: "center", marginBottom: "25px" }}>
        <select value={liga} onChange={(e) => updateParams("liga", e.target.value)} style={styles.select}>{Object.keys(DATA).map(l => <option key={l} value={l}>{l}</option>)}</select>
        <select value={temp} onChange={(e) => updateParams("temp", e.target.value)} style={styles.select}><option value="2024-2025">2024-2025</option><option value="2025-2026">2025-2026</option></select>
        <input type="number" value={jornada} onChange={(e) => updateParams("jornada", e.target.value)} style={{ ...styles.select, width: "60px" }} />
      </div>
...
const styles = {
  ...
  select: { padding: "8px", borderRadius: "8px", background: "#1E293B", color: "#FFF", border: "1px solid #334155" },
  card: { backgroundColor: "#1E293B", padding: "18px 24px", borderRadius: "14px", marginBottom: "15px", cursor: "pointer", color: "#FFF" },
  ...
  team: { fontWeight: "600" },
  score: { backgroundColor: "#0EE7AC", color: "#0F172A", padding: "5px 15px", borderRadius: "8px", fontWeight: "900" },
  ...
  closeButton: { position: "absolute", top: "10px", right: "15px", background: "none", border: "none", color: "#FFF", fontSize: "1.5rem", cursor: "pointer" },
  statsDashboard: { backgroundColor: "rgba(15, 23, 42, 0.5)", padding: "20px", borderRadius: "20px" }
};